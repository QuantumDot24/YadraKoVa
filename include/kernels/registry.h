#pragma once
#include "core/tensor.h"   // <-- reusa DType de aqui, no lo redeclares
#include <cuda.h>
#include <string>
...
// DType vive en core::DType -- una sola fuente de verdad para todo
// el proyecto. Aqui solo lo traemos al namespace kernels via alias,
// para no tener que escribir core::DType en cada firma.
using yadrakova::core::DType;
...
public:
    ...
    void register_kernel(const std::string& name, Arch arch, DType dtype, KernelBinary binary) {
        std::lock_guard<std::mutex> lock(mtx_);
        binaries_[key(name, arch, dtype)] = binary;
    }

    CUfunction get_function(const std::string& name, Arch arch, DType dtype) {
        std::lock_guard<std::mutex> lock(mtx_);
        ...
        if (!cu_init_done) { cuInit(0); cu_init_done = true; }
...
private:
    struct Entry { CUmodule module = nullptr; CUfunction function = nullptr; };

    static std::string key(const std::string& name, Arch arch, DType dtype) {
